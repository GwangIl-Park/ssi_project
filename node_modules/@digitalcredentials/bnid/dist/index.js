...
var base58Universal = require('@digitalcredentials/base58-universal');
...
function _base16Encoder({bytes, idEncoder}) {
  let encoded = util.bytesToHex(bytes);
...
function _base58Encoder({bytes, idEncoder}) {
  const encoded = base58Universal.encode(bytes);
  if(idEncoder.fixedLength) {
    const fixedBitLength = _calcDataBitLength({
      bitLength: bytes.length * 8,
      maxLength: idEncoder.fixedBitLength
  ...
  decode(id) {
    ...
    switch(encoding) {
      ...
      case 'base16upper':
        ...
        decoded = util.bytesFromHex(data);
        break;
      case 'base58':
        decoded = base58Universal.decode(data);
        break;
      default:
